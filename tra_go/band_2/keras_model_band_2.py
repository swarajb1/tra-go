...
def metric_new_idea(y_true, y_pred):
    return (
        ...
        + metric_win_checkpoint(y_true, y_pred)
    )
...
def metric_loss_comp_2(y_true, y_pred):
    ...
    # # return z_max_above_error + z_pred_valid_error + z_min_below_error + win_amt_true_error * 2 + win_amt_pred_error * 4
...
def metric_win_pred_capture_percent(y_true, y_pred):
    ...
    pred_capture = K.sum((max_pred - min_pred) * K.cast(wins, dtype=K.floatx()))
...
def metric_pred_capture_percent(y_true, y_pred):
    ...
    total_capture_possible = K.sum(max_true - min_true)
...
#         (max_pred_s - min_pred_s) * RISK_TO_REWARD_RATIO,
#     )
...
def metric_win_checkpoint(y_true, y_pred):
    ...
    total_capture_possible = K.mean(max_true - min_true)

    return (total_capture_possible - (open_trade_capture_new + pred_capture_new * 2)) * 10
